Agent::ToolRegistry.register(
  name: 'cover_audio',
  description: 'Сделать музыкальный кавер прикреплённого пользователем трека — Suno переделывает в новом стиле, сохраняя мелодию. Используй когда: к сообщению ПРИКРЕПЛЁН аудиофайл И пользователь хочет именно кавер/переделку/в стиле X (e.g. "сделай метал-кавер", "переделай как 80-е", "сделай минус", "убери вокал"). НЕ используй для добавления вокала к инструменталке (add_vocals) или генерации с нуля (compose_song). Возвращает 2 клипа.',
  parameters: {
    ...
    'prompt'        => { type: 'string', description: 'Опционально: тема/настроение/текст для нового исполнения. Если пользователь дал КОНКРЕТНЫЙ текст — передай его сюда дословно, целиком, без парафраза. Если только тему — передай её кратко, без украшательств. Если instrumental=true — оставь пустым или опиши только настроение/инструменты, НЕ описывай голос/вокал.' },
    ...
    'instrumental'  => { type: 'boolean', description: 'true если кавер должен быть БЕЗ вокала (минус, инструментал). Ставь true когда пользователь говорит "минус", "инструментал", "без вокала", "без слов", или когда исходный трек инструментальный и пользователь не просит добавить вокал. По умолчанию false (с вокалом).' },
