...
module ImageGen
  ...
  class AtlasAdapter < Adapter
    ...
    PROMPT_TEMPLATE = <<~PROMPT.freeze
      Ты — эксперт по промптам для AI image generator (текущая модель — %{model_name}).
      ...
      %{knowledge}
      Правила:
      - ГЛАВНОЕ: язык промпта = язык запроса пользователя. Если запрос по-русски — промпт по-русски, если по-английски — по-английски. Модель нативно понимает оба языка — НЕ переводи.
      ...
      - Модель хорошо отрабатывает развёрнутые описательные промпты (несколько предложений ок). Порядок деталей не критичен — модель не приоритизирует начало так сильно, как FLUX. Описывай связно, как живой текст.
    ...
    EDIT_PROMPT_TEMPLATE = <<~PROMPT.freeze
      Ты — эксперт по промптам для AI image generator в режиме редактирования картинок (текущая модель — %{model_name}).
