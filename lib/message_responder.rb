...
class MessageResponder
  ...
  def respond
    ...
    if cmd =~ /^\s*(ублюдки\s+|бот[,]?\s*(скажи|зачитай)\s+)(?<voice>ганс)?\s*(минус)?\s*(?<track>\d+)?\s*((?<horoscope>гороскоп.*)|(?<text>.*))/im
      ...
      horoscope = Regexp.last_match[:horoscope]
      ...
      if horoscope && !horoscope.empty?
        text = Horoscope.new(@user.name).predict!
      end

      filename = Polly.new(text, voice: voice_id, minus: minus, track_id: track_id).generate
      bot.api.sendVoice chat_id: @chat_id, voice: "https://42fm.ru/bot-kek1488191929173/#{filename}"
      return
    end
    ...
    when /^(бот[,]?\s+)(?<text>((почему|зачем|как|когда|что|где|сколько|какой|расскажи|ответь)\s.*)|(.*[?]$))/im
      text = Regexp.last_match[:text]
      context = get_chat_context
      res = "@#{user.name} #{GptMaster.call(text, context: context)}"
    when /^(бот[,]?\s+)?([.]|балаболь|жпт)?\s+(?<text>.*)/im
      text = Regexp.last_match[:text]
      context = get_chat_context
      res =  "@#{user.name} #{GptMaster.call(text, context: context)}"
    when /^(бот[,]?\s+)?(жпт4)\s+(?<text>.*)/im
      text = Regexp.last_match[:text]
      context = get_chat_context
      model = 'chatgpt-4o-latest'
      res =  "@#{user.name} #{GptMaster.call(text, context: context, model: model)}"
  ...
  def process_voice_message
    return false if not Settings.auth['audio_chat_ids'].include? @chat_id
