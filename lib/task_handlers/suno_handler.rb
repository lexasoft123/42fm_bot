class SunoTaskHandler
  ...
  def poll_and_deliver(task, api)
    ...
    LOGGER.debug "SunoTaskHandler[#{task.id}]: polling #{task.external_id} (attempt #{task.attempts + 1}/#{task.max_attempts})"
  ...
  def get_chat_context(chat_id)
    ...
    LOGGER.warn "SunoTaskHandler context: #{e.message}"
    ''
  end
  ...
  def get_relevant_knowledge(query, chat_id)
    ...
    LOGGER.warn "SunoTaskHandler knowledge: #{e.message}"
    ''
  end
