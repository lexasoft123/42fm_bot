...
│   │   ├── atlas_adapter.rb   # Atlas Cloud (multi-model: Qwen Image 3 Pro default / Nano Banana / Wan / ...)
...
**Audio source resolution:** `GptChat#attached_audio` delegates to `AudioAttachment.resolve` (`lib/audio_attachment.rb`) — (1) the current message's audio/voice/audio-MIME document (`source: :message`); (2) the reply target's (`:reply`); (3) a lookback over stored user rows (`:lookback`, plus `age_min` and `uploader_uid`): last `LOOKBACK_ROWS` (20) rows AND at most `LOOKBACK_MAX_AGE_MIN` (30) minutes old, thread-filtered only in real forum chats (`chat.is_forum` — in non-forum supergroups `message_thread_id` is just a reply-chain root). The 30-minute bound exists because rows alone didn't bound staleness: prod 2026-08-24 a quiet DM's 20-row window reached a 12-day-old upload and the wrong song was covered; 2026-09-04 a 7-hour-old group upload turned "спой песню" into a cover. `Agent::Runner` passes `audio_source` (plus `user_initiated` and `forum_thread_id`) into `tool_ctx` so a tool can tell an attachment on the request from an implied one — `separate_vocals` uses it; `cover_audio`/`add_vocals` still take any resolved audio as their upload — and words `audio_hint` by source: `:message`/`:reply` → "[К сообщению прикреплён аудиофайл …]"; `:lookback` → "[К этому сообщению аудио НЕ прикреплено. В чате N мин назад <кто> прислал(а) …, используй только если запрос явно про этот трек, иначе спроси]".
...
- `SunoSeparateVocalsHandler` (`suno_separate_vocals`) — Suno vocal/stem separation → poll → deliver stems as audio media groups (see "Suno vocal/stem separation"). Triggered via the `separate_vocals` agent tool.
...
- `separate_vocals(type:, audio_url: | task_id:+audio_id:, stem_name:)` / `poll_separation_once(task_id)` — vocal/stem separation (see "Suno vocal/stem separation")
...
#### Upload-source failures (cover / add-vocals)
`SunoTaskHandler#handle_generation_failure` handles `{ generation_failed: }`: a `suno_generate` song resubmits (up to `MAX_GENERATION_RETRIES`, whether or not Suno gave a reason), but `suno_cover_audio` / `suno_add_vocals` **fail immediately** (`suno_source_rejected`) — prod 2026-07-20..09-13 had 22 of 38 upload tasks fail, with immediate resubmits of the same file failing in a row (one file: 12 submits across 3 tasks) while the same recording went through weeks later; the blind resubmits ended with the agent silently composing a new song instead of the cover. `:retry` (SUCCESS without clips) still resubmits for every type. Every resubmit clears `external_id` AND resets `attempts` to 0, so each fresh job gets its own poll budget (prod task 3715 timed out mid-resubmit on a shared 60-poll budget).

`mark_failed_and_notify` is task-type aware: chat notice "Не удалось сделать кавер" / "Не удалось добавить вокал" / "Не удалось сгенерировать песню"; agent_event `cover_failed` / `add_vocals_failed` / `song_failed[_after_retries]`; summary for upload tasks is `Кавер «title» (task #ID, режим auto|custom) | Стиль: … | Причина: … | <Suno detail>` (the song summary's topic falls back past empty strings to request/theme/title). The "don't swap a cover for a from-scratch song without consent; offer a later retry / another file / lyrics" directive lives in `AgentEventHandler::EVENT_DESCRIPTIONS`, not in the 600-char-capped summary.

**Retrying later:** `cover_audio` / `add_vocals` accept optional `retry_of_task_id` (the `BackgroundTask.id` from the event summary) — accepted only for a **failed** task of the same type in the same chat, and refused when a `pending`/`done` task of that type already carries the same `retry_of_task_id` (a failed task stays failed forever, so a user "повтори" plus a cron intention from a rate-limited retry would otherwise bill twice; same `json_extract` dedup as `maybe_chain_cover_art`); it copies the source and any params the call leaves empty (explicit args win), so a cron-driven retry works with no `ctx[:audio]`. New upload tasks store `upload_file_id`; `SunoTaskHandler#fresh_upload_url` resolves a fresh Telegram link at submit time (getFile links are only guaranteed ~1 h), falling back to the stored `upload_url` for agent-supplied links / legacy rows.

#### DM audio follow-up — `lib/pending_audio_request.rb`
Prod 2026-08-24: a user wrote "сделай аранжировку этой песни" and sent the file a minute later; captionless audio never reaches the agent, so the request ran against a stale source. When `cover_audio` / `add_vocals` / `separate_vocals` find no source, the bucket isn't exhausted, and the turn is a real user turn in a private chat (`tool_ctx[:private_chat]`, `[:user_initiated]`, `[:request_text]`, `[:message_id]` — passed by GptChat → Runner), `PendingAudioRequest.offer` stores `{text, message_id, tool}` under `[chat_id, uid]` (in-memory, mutex, 10-min TTL) and the tool returns a **plain string** asking for the file — not `ToolResult.deferred`, so no scratchpad intention is left behind to be redone later. `MessageResponder#run_pending_audio_followup` takes the entry (once) when the user's next captionless audio / audio document arrives and runs `Commands::GptChat` with `cmd` = the stored text. Guards: DM only (an unrelated track in a group must not spend credits), voice notes never fire it, edits never fire it, other users can't consume it, any `GptChat#execute` by that user clears the entry first (a new question cancels the pending request), and every Suno tool calls `PendingAudioRequest.clear_for(ctx)` right before `BackgroundTask.create!` (a turn can offer the follow-up and then still create a task in a later iteration). A restart forgets pending entries.
...
#### Suno vocal/stem separation
`separate_vocals` agent tool (`lib/agent/tools/separate_vocals.rb`), `suno_separate_vocals` task type, `SunoSeparateVocalsHandler`. Uses `POST /api/v1/vocal-removal/generate` + `GET /api/v1/vocal-removal/record-info`. Unlike `cover_audio instrumental=true` (which regenerates music in a new style), separation cuts tracks out of the ORIGINAL recording.

- **Modes** (tool `mode` → Suno `type`): `vocals` → `separate_vocal` (vocals + instrumental, 10 credits, default); `stems` → `split_stem` (up to 12 stems, 50 credits, only on explicit request); `instrument` → `split_stem_advanced` + `stemName` (one named instrument, 20 credits). `stem_name` is a free string validated case-insensitively against `SunoClient::STEM_NAMES` (the ~100-name enum is not shipped in the schema); a miss returns close suggestions and creates no task.
- **Source** — EITHER a Suno clip (`taskId` + `audioId`, resolved in the handler via `fetch_audio_ids` + `clip_index`) OR any audio URL (`audioUrl`, ≤20 MB); never both. Tool resolution order (`SeparateVocalsTool.resolve_source`): explicit `suno_task_id` → explicit `source_message_id` (a user upload row → its file, a bot song row → its clip; no age bound) → reply to a bot song message, unless the message carries its own attachment (checked before `ctx[:audio]` because such a reply also carries the song's audio; clip index parsed from the end of the row body `[песня: T (2/2)]`) → `upload_url` → current-message/reply audio → **on real user turns only** (`tool_ctx[:user_initiated]`), the NEWEST of a lookback upload and the latest bot song row (same forum topic via `tool_ctx[:forum_thread_id]`), both ≤ `AudioAttachment::LOOKBACK_MAX_AGE_MIN` (30) → deferral. The source is resolved BEFORE the shared `suno` rate-limit check (DB-only; getFile happens after), so a rate-limited deferral's intent carries a concrete re-selecting call (`separate_vocals(mode=…, source_message_id=…)` / `suno_task_id=…, clip_index=…`) — cron-driven retries run with no audio/reply context and must not fall back to "the latest song".
- **Poll** — `successFlag` `PENDING`/`SUCCESS`/`CREATE_TASK_FAILED`/`GENERATE_AUDIO_FAILED`/`CALLBACK_EXCEPTION`. Stems come from named `response` fields (`vocalUrl`, `instrumentalUrl`, `backingVocalsUrl`, `drumsUrl`, `bassUrl`, `guitarUrl`, `keyboardUrl`, `percussionUrl`, `stringsUrl`, `synthUrl`, `fxUrl`, `brassUrl`, `woodwindsUrl`), falling back to `response.originData[]` (`audio_url`, `stem_type_group_name`, skipping the `originUrl` source track) — the only place `split_stem_advanced` output may appear (undocumented shape). `CALLBACK_EXCEPTION` with stems present counts as success (it only means Suno couldn't reach our placeholder callback). **Never `:retry`/resubmit** after Suno accepted a job — each call is billed; empty SUCCESS is a failure with detail.
- **Delivery** — `mark_done!` BEFORE sending (stem URLs, valid 14 days, land in `result`), then `sendMediaGroup` of audio in chunks of ≤10 (single-item groups are accepted by Telegram — prod add_vocals sends them), caption on the first stem, filenames `Performer_-_Title_(Vocals).mp3`, one `[стем: Title — Vocals]` bot row per sent message (no `bg_task_external_id`, so replies to a stem aren't misresolved as a song by `cover_art`/`convert_to_wav`). Delivery never raises out of the handler (an exception reaching `TaskRunner#process_one` would overwrite `:done` with `:failed`); stems that fail to download or send are reported once via `separation_delivery_failed` (chat notice only when nothing arrived).
- **Failures** — permanent submit rejections (`TaskRunner.permanent_error?`, see "Submit error format") fail immediately with `separation_failed` + detail instead of TaskRunner's raw `Ошибка:` notice; transient ones re-raise up to `MAX_SUBMIT_FAILURES` (3).
...
- **`ImageGen::FluxAdapter`** (`lib/image_gen/flux_adapter.rb`) — FLUX 2 via `api.bfl.ai`. Submit POST to `/v1/{model}`, poll GET `/v1/get_result`. Uses `safety_tolerance: 5` and `output_format: 'jpeg'`. Auth header is `x-key`. NOT built on `ModelProviderClient` (different host + auth scheme). Has a one-release back-compat shim that reads top-level `Settings.flux` if `image_gen.providers.flux` is absent — removed once prod settings.yml is migrated.
- **`ImageGen::AtlasAdapter`** (`lib/image_gen/atlas_adapter.rb`) — Atlas Cloud (default t2i: `qwen-image-3.0-pro/text-to-image`, default edit: `qwen-image-3.0-pro/edit`, both configurable; per-request `model:` overrides). Submit POST to `/api/v1/model/generateImage`, poll GET `/api/v1/model/prediction/{id}`. **Request body is FLAT**. **T2I sizing is per-model**: Wan/nano-banana send `{width, height}` integers; Seedream and Qwen Image 3.0 send `{size: 'WIDTH*HEIGHT'}`; GPT Image 2.5 Sunburst sends `{size: 'WIDTHxHEIGHT'}`. The **edit image field is per-model** (getting it wrong makes the model silently ignore the source and regenerate): Wan reads singular `image: 'data:…;base64,…'`; the **nano-banana, Seedream and GPT Image families read plural `images: ['data:…', …]`**; **Qwen Image 3.0 reads `reference_image_urls: ['data:…', …]`** and accepts at most three references (the adapter trims extras). All accept base64 data URIs without an upload step. Atlas's published `input.{...}` example shape silently fails — submit returns 200+id but poll instantly returns masked "Field required" — so we use the flat shape, confirmed via live probe (nano-banana `images` shape confirmed 2026-06-24; Qwen and GPT Image fields/limits verified against Atlas docs 2026-09-22). Status mapping: **non-200 poll → `:poll_error`** (incl. the swallowed `[nil,nil]` SSL/timeout — Atlas's status endpoint 500s persistently for some FAILED predictions, so a non-200 is NOT "processing"), `processing|queued` → `:pending`, `completed|succeeded` → `{url:}`, `failed` → `:failed`, unknown HTTP-200 status → log once + `:pending`. `ImageGenTaskHandler#poll_and_deliver` fails the task (+ notifies) after `MAX_POLL_ERRORS` (5 ≈ 75s) **consecutive** `:poll_error`s (`task.params['poll_errors']`, reset on any healthy poll) instead of spinning to the 60-attempt (~15min) timeout with a misleading "таймаут". Response wraps under `data.{...}`; adapter accepts both wrapped and unwrapped defensively. Submit-response id resolution accepts both `data.id` (canonical) and top-level `id`.
- **`ImageGen::CloseRouterImgAdapter`** (`lib/image_gen/closerouter_adapter.rb`) — CloseRouter Nano Banana Pro (Google) via `api.closerouter.dev`. Default models: `google/nano-banana-pro` (T2I) and `google/nano-banana-pro-edit` (image edit) — two separate model ids, NOT a flag on the base model. **Synchronous**: single `POST /v1/images/generations` returns `{data: [{url: 'https://cdn/...png', ...}]}` directly; adapter extracts `data[0].url` and returns `{url:, completed:true}`. Edit body uses plural `images: ['data:image/<type>;base64,<b64>', …]` (array; Nano Banana Pro combines multiple input images, so all resolved sources are mapped in). `synchronous? = true` so `ImageGenTaskHandler` skips the poll cycle.
- **`ModelProviderClient`** (`lib/model_provider_client.rb`) — generic Bearer+JSON HTTP wrapper (formerly `AtlasClient`; renamed because it's no longer Atlas-specific). Constructor takes a config dict (`api_url`, `api_key`) + `tag:` for greppable logs (`'AtlasImg'`, `'CloseRouterImg'`, future `'CloseRouterVideo'`). Asymmetry: `post` raises on non-2xx (so handler `bail_or_retry` engages); `get` returns `[code, body]` and swallows `OpenSSL::SSL::SSLError`/`Net::OpenTimeout`/`Errno::ECONNRESET` as `[nil, nil]` (which the adapter maps to `:poll_error`, absorbed by the handler's consecutive-error tolerance). `post` does NOT swallow SSL errors — preserves FluxAdapter's existing behavior; submit-side TLS resilience can be retrofitted with a per-call retry wrapper if needed.
...
image_gen:
  ...
  default_model: qwen-image-3-pro # catalog key used when agent omits/sends unknown model
  providers:
    atlas:
      api_url: https://api.atlascloud.ai
      api_key: ...
      text_to_image_model: qwen-image-3.0-pro/text-to-image
      image_edit_model:    qwen-image-3.0-pro/edit
      width:  1024
      height: 1024
    flux:
  ...
  models:                         # agent-selectable catalog (ImageGen::Catalog); KEY = generate_image(model:) enum value
    ...
    seedream-5-pro: { provider: atlas, t2i: bytedance/seedream-v5.0-pro/text-to-image, edit: bytedance/seedream-v5.0-pro/edit, multi_image: true, desc: '...' }
    qwen-image-3-pro: { provider: atlas, t2i: qwen-image-3.0-pro/text-to-image, edit: qwen-image-3.0-pro/edit, multi_image: true, desc: '...' }
    gpt-image-2.5-sunburst: { provider: atlas, t2i: openai/gpt-image-2.5-sunburst/text-to-image, edit: openai/gpt-image-2.5-sunburst/edit, multi_image: true, desc: '...' }
