...
- **Bot replies in context:** `MessageResponder#deliver` persists a `messages` row for **every** successfully-sent bot output — text, sticker (`[стикер]`), image (`[картинка]`), and voice (`[голос]`) — not just GPT replies. Text uses the `message_id` `MessageSender#send` returns; media reuse `Message.persist_bot_reply(response:)` — the centralized bot-side write path, also used (via thin wrappers) by all background-task handlers; it captures `attachment_photo_file_id` from photo sends so the agent can re-view bot-generated images via `view_image`. This means the agent's context sees the bot's own non-GPT output, and a user reply to *any* bot message (a sticker, a search image, etc.) resolves: `get_chat_context` backfills the reply target from DB when it's outside the recent window. `get_chat_context` serializes rows as JSON objects `{id, role, who, msg, [reply_to], [thread], [fwd], [edited], [audio], [audio_meta], [photo]}`. The `load_messages` agent tool lets the agent pull a window around any anchor id. (The legacy `persist_as_bot_reply` opt-in flag was removed; persistence is now unconditional.)
...
- **Vision/Image recognition:** When a user attaches or replies to a photo with "бот ...", `GptChat` downloads the photo (`TelegramFile.download_image`), base64-encodes it, and passes it to the agent as a multi-modal message. The agent can describe, analyze, and answer questions about images. Falls back to text-only if download fails.
- **Historical images (`view_image` tool):** incoming photos are persisted as `messages.attachment_photo_file_id` (photo-only rows get body `[фото]`), and bot-sent images (image_gen, Suno cover art, `deliver :image`) are captured from the send response too; context rows show `photo: true`. The agent calls `view_image(message_id:)` to fetch one — the Runner injects it as a vision block after the iteration's tool results and upgrades the turn to `agent_vision` (only when both settings share `api_type`; otherwise `ctx[:can_view_image]` is false and the tool degrades to text). Only photos received after the feature deployed are fetchable; document-images (`image/*` MIME files) aren't captured. Detail: `docs/architecture.md` "Historical images".
...
| Image generation (FLUX / Atlas Cloud / CloseRouter Nano Banana Pro) | `lib/image_gen/*.rb` + `lib/model_provider_client.rb` + `lib/task_handlers/image_gen_handler.rb` + `lib/agent/tools/image_gen.rb` (agent-only, no direct command). Backend chosen by `Settings.image_gen['provider']` (`flux` / `atlas` / `closerouter`); provider snapshotted into `task.params['provider']` at submit time. CloseRouter is synchronous — handler short-circuits poll cycle via `Adapter#synchronous?`. |
