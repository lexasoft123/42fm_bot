...
- **Bot replies in context:** `MessageResponder#deliver` persists a `messages` row for **every** successfully-sent bot output — text, sticker (`[стикер]`), image (`[картинка]`), and voice (`[голос]`) — not just GPT replies. Text uses the `message_id` `MessageSender#send` returns; media reuse `Message.persist_bot_reply(response:)`. This means the agent's context sees the bot's own non-GPT output, and a user reply to *any* bot message (a sticker, a search image, etc.) resolves: `get_chat_context` backfills the reply target from DB when it's outside the recent window. `get_chat_context` serializes rows as JSON objects `{id, role, who, msg, [reply_to], [thread], [fwd], [edited], [audio], [audio_meta]}`. The `load_messages` agent tool lets the agent pull a window around any anchor id. (The legacy `persist_as_bot_reply` opt-in flag was removed; persistence is now unconditional.)
