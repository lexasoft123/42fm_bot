...
- **Image watchdog (`Runner#run`)** — the model sometimes promises/describes an image but never calls `generate_image`, so nothing is delivered (silent). Two triggers nudge it once per turn to actually call the tool: (1) `hallucinated_image_caption?` — output text imitates the `🎨 <caption>` format (keys off model OUTPUT); (2) `image_request_unanswered?` — the user's request matches `DRAW_DIRECTIVE` (imperative нарисуй/дорисуй/… stems, Cyrillic-stem match since Ruby `\b` is ASCII-only) but the turn ended with no `generate_image` call (keys off `@text`). **Trigger (2) is gated to `@user_initiated` real user turns** (gpt_chat/gpt_question): the `agent_event` path passes `user_initiated: false` because its synthetic prompt echoes the original `Запрос: …` into `@text` and would otherwise re-trigger image-gen on the event loop. `DRAW_DIRECTIVE` deliberately excludes `сгенерируй` (generic "generate" — пароль/текст/идею aren't images).
...
- **Tool ctx provenance keys** — `ctx[:audio_source]` (`:message`/`:reply`/`:lookback`, from `AudioAttachment`), `ctx[:user_initiated]` (false on agent_event/cron turns) and `ctx[:forum_thread_id]` (set only in forum chats). Tools that pick an IMPLIED target (no reply/attachment/explicit id) must require `user_initiated` — synthetic turns have no audio/reply context, so "latest X" there is a guess that can bill the wrong thing — and should put a concrete re-selecting argument (message_id / task id) into any deferred intent. `separate_vocals` is the reference implementation.
