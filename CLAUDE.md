...
| Suno upload-cover (musical cover) | `lib/agent/tools/cover_audio.rb` → `suno_cover_audio` task (handled by `SunoTaskHandler`); 2 clip output. Tool exposes `lyrics` (verbatim → custom-mode) + `topic` (auto-gen seed → non-custom-mode). See `cover_audio` gotcha for the full lyrics/topic/legacy/title resolution chain, instrumental override, and reply-target source-lyrics auto-fallback. |
