...
- Suno cover-art uses a separate handler (`SunoCoverArtHandler`) since its output is images, not audio; polls via `SunoClient#poll_cover_art_once` (same `/record-info` endpoint, different success-branch extraction); failures emit `cover_art_failed` agent_event so the agent can comment.
