...
- **Radio keepalive** — `Radio#start_keepalive` (called from `lib/bot.rb` at startup, alongside TaskRunner/CronScheduler) spawns a background thread that pings Liquidsoap every `KEEPALIVE_INTERVAL` (20s) with `request.queue`. Liquidsoap closes idle telnet connections; reusing a stale socket loses the first command's response, which made `!track` return `(нет данных)` *while a track was actually playing* (verified on prod: metadata was available via a fresh telnet probe, but every logged `!track` hit the stale-socket reconnect race). The keepalive pays the reconnect-on-idle penalty on the throwaway ping so real user commands always hit a warm socket. Idempotent (`@keepalive ||= …`), so safe across the bot's crash/retry loop. The interval must stay well under Liquidsoap's idle-close timeout. `Radio#command`'s reconnect rescue catches `Timeout::Error` (in addition to `Errno::EPIPE`/`ECONNRESET`/`IOError`) so a *hung* half-open socket also resets `@sock` and reconnects instead of timing out forever.
- **Radio metadata degradation** — fallback for genuinely-empty metadata: `Radio#track` returns `"сейчас ничего не играет"` (not `"(нет данных), осталось …"`) and `Radio#queue` drops slots whose per-request metadata is unresolvable (all-blank → `nil` → `RadioQueue` renders `"нихуя нет"`). With the keepalive in place this should rarely trigger; it's a safety net, not the primary fix.
