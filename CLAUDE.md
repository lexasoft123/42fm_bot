...
- Suno uses V5 model; tags are enriched via LLM — artist names are **never** included in tags (Suno blocks them), instead describe the sound characteristics. `TAGS_PROMPT` orders tags *genre → mood → instruments → vocals → mix*; negatives flow through the structured `negative_tags` param on `compose_song`/`add_vocals`/`cover_audio` (shared description: `SUNO_NEGATIVE_TAGS_DESC` in `_suno_language_rule.rb`), never inlined into `tags`. Tag-ordering, lyric-craft directives, expanded section-marker palette: `docs/architecture.md#tag-ordering-convention-tags_prompt` and `#lyric-craft-directives-lyrics_prompt`.
